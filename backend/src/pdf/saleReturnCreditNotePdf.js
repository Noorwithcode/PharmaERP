...
const drawHeader = (
  ...
  const companyName = safeText(
    (process.env.PHARMACY_NAME || process.env.COMPANY_NAME),
    "PHARMAERP PHARMACY"
  );

  const companyAddress = safeText(
    (process.env.PHARMACY_ADDRESS || process.env.COMPANY_ADDRESS),
    "Medicine Billing, Inventory and Pharmacy Management"
  );

  const companyPhone = safeText(
    (process.env.PHARMACY_PHONE || process.env.COMPANY_PHONE),
    "-"
  );

  const companyEmail = safeText(
    (process.env.PHARMACY_EMAIL || process.env.COMPANY_EMAIL),
    "-"
  );

  const companyGstin = safeText(
    (process.env.PHARMACY_GSTIN || process.env.COMPANY_GSTIN),
    "-"
  );
...
const renderSaleReturnCreditNote = (
  outputStream,
  data
) => {
  return new Promise(
    (resolve, reject) => {
      ...
      const doc =
        new PDFDocument({
          ...
          info: {
            Title:
              `Sale Return Credit Note ${safeText(
                returnHeader.returnNumber
              )}`,
            Author:
              safeText(
